...
The \t{path} of a car depends on where it starts and its randomly chosen destination. Its \t{initialPosition} is the same as the location of its point of departure, thus although the \t{path} and \t{initialLocation} differ per vehicle they are dependent within a vehicle. \baakman{`differ per vehicle but dependent within a vehicle'? Ik begrijp dit niet helemaal.}
